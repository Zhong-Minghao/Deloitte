import sys
from home_page_structure import Ui_MainWindow
from manual_selection_page_1 import Ui_manual_selection_page
from table_widget_1 import Ui_Form
import pandas as pd
import numpy as np
import os
...
# 读取原始数据
df1 = read_df('df_factor_01_20220822')
df2 = read_df('df_factor_02_20220822')

#
df = pd.read_excel(r"./[2] 阈值统计_v" + f"{version}_{date}.xlsx", header=0)
...
#
df_stats = pd.read_excel(r"./[3] 数据统计_v" + f"{version}_{date}.xlsx", header=0).rename(columns={
    "行业": "industry",
    "指标": "factor"
})
...
#
df_combine = pd.merge(df1, df2, left_on=['BASIC_entity_name', 'BASIC_year', 'PROFILE_industry'],
...
def alter_factor(this_factor):
    ...
    if '_02_' in this_factor:
        this_factor = this_factor.replace('_02_', '_')
    if '_01_' in this_factor:
        this_factor = this_factor.replace('_01_', '_')
    return this_factor_origin, this_factor


def get_anomaly_rate(series, series_direction, low_num, up_num):
    """计算阈值"""
    if series_direction == '向上':
        anomaly_proportion = (series[series > up_num].count()) / series.count()
    elif series_direction == '向下':
        if low_num == '0.01且>0':
            anomaly_proportion = (series[(series < 0.01) & (series > 0)].count()) / series.count()
        elif low_num == '-':
            anomaly_proportion = '-'
        else:
            anomaly_proportion = (series[series < low_num].count()) / series.count()
    else:
        if low_num == '0.01且>0':
            anomaly_proportion = (series[series > up_num].count() +
                                  (series[(series < 0.01) & (series > 0)].count())) / series.count()
        elif low_num == '0.005且>0':
            anomaly_proportion = (series[series > up_num].count() +
                                  (series[(series < 0.005) & (series > 0)].count())) / series.count()
        else:
            anomaly_proportion = (series[series > up_num].count() +
                                  series[series < low_num].count()) / series.count()
...
def graph_figures(my_industry, my_factor, indicator, pct_low, pct_up, pct_new_low=np.nan, pct_new_up=np.nan):
    ...
    print(pct_low, pct_up, pct_new_low, pct_new_up)
    plt.close()
...
def next_item():
    ...
    if ui.select.num == -1:
        ...
        print(ui.select.artificial_lower)
        try:
        ...
        df.to_excel(root_path + '人工判定结果.xlsx')
...
def save_tmp_result():
    ...
    ui.select.listView.clear()
    filename = f"tmp_人工判定结果{time.strftime('%Y%m%d-%H%M%S')}.xlsx"
    df.to_excel(root_path + '/' + filename)
    ui.select.current_industry_display.setText('保存结果成功')
...
class This_Window(QWidget, Ui_manual_selection_page):
    # 人工选择页面
    def __init__(self):
        super().__init__()
        self.chose_range = ''
...
class FrameContactPage(QWidget, Ui_Form):
    # 阈值概览页面
    def __init__(self):
        super().__init__()
        self.setupUi(self)
