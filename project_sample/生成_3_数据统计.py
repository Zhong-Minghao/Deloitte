...
# k值计算的分母
my_k_denominator = Py1.my_k_denominator

# 调整零的个数至所有数字的比例
targeted_zero_percentage = Py1.targeted_zero_percentage
asserted_zero = Py1.asserted_zero

# LOF初筛使用阈值
initial_filter_m = Py1.initial_filter_m
...
# m值（从大到小排序）
m = [8, 7.5, 7, 6.5, 6, 5.5, 5, 4.5, 4, 3.5, 3, 2.5]
...
def save_df(df_name, output_name):
    exec(f"{df_name}.to_excel(r'./{output_name}.xlsx', index=True, float_format='%.16f')")


# 读取数据
df_1 = Py1.read_df(original_data_1)
df_2 = Py1.read_df(original_data_2)
...
# 读取字段
df_indicator = pd.read_excel(Py1.old_range_data, header=1)

df_indicator = df_indicator.set_index(['细分行业', '指标代码'])

industries_list = [x for x in list(df['PROFILE_industry'].unique()) if x != '' and not pd.isna(x)]

this_columns = ['行业', '指标', '空值个数', '全样本个数', '基于非空原始样本->', '均值', '中位数', '方差', '0值原始个数', '0值剔除个数',
                '极端值个数', '有效样本个数', '基于有效样本->']\
               + [str(x)+'%' for x in percentile_list]\
               + [str(x) for x in standard_deviation_list]\
               + [f'm={str(x)}:下限' for x in m]
m.reverse()
this_columns = this_columns + [f'm={str(x)}:上限' for x in m]

output_df = pd.DataFrame(columns=this_columns)
...
output_df.to_excel(r"./output/[3] 数据统计_v" + f"{version}_{date}.xlsx", index=False)
